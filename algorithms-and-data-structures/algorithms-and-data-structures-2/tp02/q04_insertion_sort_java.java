...
class ColecaoRestaurantes{
    ...
    public void ordenacaoInsercaoCidade(Restaurante[] restaurantes){
        for(int i=1;i<restaurantes.length;i++){
            Restaurante chave=restaurantes[i];
            int j=i-1;  
            while(j>=0 && chave.getCidade().compareTo(restaurantes[j].getCidade())<0){
...
public class q4 {  
    public static void main(String[] args) throws Exception{
        ...
        Restaurante[] selecionadosFinal=new Restaurante[k];
        for(int l=0;l<k;l++){
            selecionadosFinal[l]=selecionados[l];
        }
        long inicio=System.currentTimeMillis();
        colecao.ordenacaoInsercaoCidade(selecionadosFinal);
        long fim=System.currentTimeMillis();
        long tempo=fim-inicio;
        FileWriter fw=new FileWriter("matricula_insercao.txt");
        fw.write("886130 + tempo");
        fw.close();
        for(int p=0;p<selecionadosFinal.length;p++){
            System.out.println(selecionadosFinal[p].formatar());
        }
